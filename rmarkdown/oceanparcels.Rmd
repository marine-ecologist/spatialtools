...
Python code from Ani *et al* (2024) "[Connectivity modelling identifies sources and sinks of coral recruitment within reef clusters](https://www.nature.com/articles/s41598-024-64388-8)" *Scientific Reports* 13564

-   changes to work with OceanParcels 3.0.4
...
df_filtered.to_csv('oceanparcels_seeding.csv', index=False)
...
Import habitat map and particle points from oceanparcels simulation, create particle tracks
...
Visualise ~200 subset tracks across the 5 days
...
map two particle tracks with contrasting dispersal distances:
...
Calculate track lengths for all particles
...
Calculate the total distances per particle per day (500m distance bins):
...
ocean_parcels_movement_m <- ocean_parcels_movement |> 
  ...
  mutate(length_bin = cut(length_m, 
                          breaks = seq(0, 20000, by = 250), 
                          include.lowest = TRUE, 
                          right = FALSE, 
                          labels = paste0(seq(0, 1975, by = 250), "-", seq(250, 20000, by = 250))))


ocean_parcels_movement_bin <- ocean_parcels_movement_m |> 
  group_by(length_bin, nday) |> 
  summarise(count=n()) |> 
  mutate(proportion=count/nrow(ocean_parcels_movement_m) *100) 


ggplot() + theme_bw() + #xlim(0,20000) +
  geom_col(data=ocean_parcels_movement_bin, aes(x=length_bin, y=proportion, fill=as.numeric(length_bin)), 
           color="black",  linewidth=0, show.legend=FALSE) +
  scale_fill_distiller(palette = "RdYlBu", direction = 1) +
  xlab("\n particle distance (m)") +
  ylab("% particles") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  ggtitle("proportion of tracks per day") +
  facet_wrap(~nday) +
...
ocean_parcels_movement_m <- ocean_parcels_movement |> 
  ...
  mutate(length_m = as.numeric(length_m)) |> 
  mutate(length_bin = cut(length_m, 
                          breaks = seq(0, 20000, by = 100), 
                          include.lowest = TRUE, 
                          right = FALSE, 
                          labels = paste0(seq(0, 19900, by = 100), "-", seq(100, 20000, by = 100))))


ocean_parcels_movement_bin_subset <- ocean_parcels_movement_m_subset |> 
  group_by(length_bin, nday) |> 
  summarise(count=n()) |> 
  mutate(proportion=count/nrow(ocean_parcels_movement_m_subset) *100) |> 
  mutate(xlab = seq(0, max(ocean_parcels_movement_m_subset$length_m, na.rm = TRUE), by = 500))


ggplot() + theme_bw() + #xlim(0,20000) +
  geom_col(data=ocean_parcels_movement_bin_subset, aes(x=length_bin, y=proportion, fill=as.numeric(length_bin)), 
           color="black",  linewidth=0.25, show.legend=FALSE) +
  scale_fill_distiller(palette = "RdBu", direction = 1) +
  xlab("\n particle distance (m)") +
  ylab("% particles") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
  ggtitle("proportion of subset tracks <500m") +
  facet_wrap(~nday)
...
-  create a "static" category where movement is <10 metres in 1 day
- for each particle, calculate distance per day
- if <10m, classify "static", if >10m, classify "dispersed"

Calculate the proportion of days where the entire day is "static" (i.e. <10m distance movement):

```{r, fig.width=7, fig.height=4, message=FALSE, warning=FALSE}

static_summary <- ocean_parcels_movement %>%
  as.data.frame() %>%
  group_by(trajectory) %>%
  summarise(static_count = sum(beach == "static"), dispersed_count = sum(beach == "dispersed")) |> 
  group_by(trajectory) %>%
  mutate(total_count=sum(static_count) + sum(dispersed_count))
