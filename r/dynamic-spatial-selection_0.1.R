 deployApp(appDir="/Users/rof011/leaflet-draw-rotate/apps/",
                      appPrimaryDoc="dynamic_spatial_selection.R", appName="spatial-decision-tool", appTitle="spatial-decision-tool")
...
lizard_map <- st_read("data/geomorphic.geojson", quiet = TRUE, crs = 4326) |>
...
### create radial raster from distance to LIRS

LIRS <- st_sf(geometry = st_sfc(st_point(c(145.448641, -14.700424)), crs = 4326))


create_radial_raster <- function(center_point, res, xmin, xmax, ymin, ymax, buffer = 5000){
  # Convert resolution to degrees
  res_deg <- res / 111000
  
  # Create an empty raster with the specified boundaries and resolution
  raster_template <- rast(nrows = round((ymax - ymin + 2 * buffer / 111000) / res_deg),
                          ncols = round((xmax - xmin + 2 * buffer / 111000) / res_deg),
                          xmin = xmin - buffer / 111000, xmax = xmax + buffer / 111000,
                          ymin = ymin - buffer / 111000, ymax = ymax + buffer / 111000,
                          crs = "EPSG:4326")
  
  # Calculate the distance from each cell to the center point
  dist_raster <- distance(raster_template, center_point)
  
  # Create breaks for concentric circles
  breaks <- seq(0, buffer, by = 100)
  
  # Create a classification matrix
  rcl <- matrix(c(breaks[-length(breaks)], breaks[-1]), ncol = 2)
  
  # Create concentric circles by classifying the distances
  circle_raster <- classify(dist_raster, rcl = rcl, include.lowest = TRUE)
  
  return(circle_raster)
}
...
#radial
#radial_raster_resampled <-  resample(radial_raster, r1_bins)
#radial_resampled <- cut_raster_bins(radial_raster_resampled)
...
ui <- fluidPage(
  tags$head(includeScript('/Users/rof011/leaflet-draw-rotate/leaflet-draw-rotate.js', 'type' = 'text/javascript', 'data-unique-tag' = 'unique'),
            includeScript('/Users/rof011/leaflet-draw-rotate/L.Path.Transform.js', 'type' = 'text/javascript', 'data-unique-tag' = 'unique'),
            includeScript('/Users/rof011/leaflet-draw-rotate/Edit.Rectangle.Rotate.js', 'type' = 'text/javascript', 'data-unique-tag' = 'unique')),
  tags$style(HTML("
    .center-container {
      display: flex;
